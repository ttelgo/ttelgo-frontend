  ...
  useEffect(() => {
    const fetchBundles = async () => {
      ...
      try {
        ...
        const countryIso = getCountryIso(country.name)
        ...
        if (countryIso) {
          ...
          console.warn('No ISO code found for country:', country.name)
          // If no ISO code found, try to get all bundles (fallback)
          const allBundles = await plansService.getBundles()
          // Filter bundles by country name as fallback
          const filteredBundles = allBundles.filter(b => 
            b.countryName?.toLowerCase() === country.name.toLowerCase() ||
            b.countryIso?.toLowerCase() === country.name.toLowerCase()
          )
          setBundles(filteredBundles.length > 0 ? filteredBundles : allBundles)
        }
      } catch (err) {
                  ...
                  {bundles.length > 0 ? (
                    bundles.map((bundle) => (
                      <button
                        ...
                        className="w-full flex items-center justify-between p-4 border-2 rounded-lg transition-all border-telgo-red bg-red-50 shadow-md hover:shadow-lg"
                      >
                        <div className="flex items-center gap-4">
                          <div className="w-5 h-5 rounded-full border-2 flex items-center justify-center border-telgo-red bg-telgo-red">
